...
void Skydome::PopulateBuffers(int NumRows, int NumCols, float Radius)       //Kreiranje sfera
{
    ...
    float PitchAngle = 90.0f / (float)NumRows;      //Vertikalno vo sferata
    float HeadingAngle = 360.0f / (float)NumCols;   //Horziontalno vo sferata
    ...
    for (float Heading = 0.0f ; Heading < 360.0f ; Heading += HeadingAngle) {
        ...
        Vector3f Pos1;
        Pos1.InitBySphericalCoords(Radius, Pitch + PitchAngle, Heading + HeadingAngle);     //cartesian --> spherical coordinates
        Vertex v1(Pos1);
        Vertices[i++] = v1;
...
void Skydome::Render(const BasicCamera& Camera)
{
    ...
    glDepthFunc(GL_LEQUAL);     //Skydome da se renderira posle site drugi objekti
