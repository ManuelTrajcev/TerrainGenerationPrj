...
void GeomipGrid::InitIndices(std::vector<unsigned int>& Indices)
{
    ...
    for (int z = 0; z < m_patchSize - 1; z += 2) {      //kreriam triangle fan preku trianles (ne koristime GL_TRIANGLE_FAN, za da moze proektot da se migrira vo directx)
...
void GeomipGrid::Render()
{
    ...
    for (int z = 0; z < m_depth - 1; z += (m_patchSize - 1)) {      //povik na glDrawElementsBaseVertex    za sekoj patch
        for (int x = 0; x < m_width - 1; x += (m_patchSize - 1)) {
            int BaseVertex = z * m_width + x;
