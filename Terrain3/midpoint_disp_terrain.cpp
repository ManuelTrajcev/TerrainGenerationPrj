...
void MidpointDispTerrain::CreateMidpointDisplacement(int Size, float Roughness, float MinHeight, float MaxHeight) {
	if (Roughness < 0.0f) {
...
void MidpointDispTerrain::CreateMidpointDisplacementF32(float Roughness) {
	int RectSize = CalcNextPowerOfTwo(m_terrainSize);	//Za da funkcionira pravilno sekogas treba da bide 2^x
	float CurHeight = (float)RectSize / 2.0f;
	float HeightReduce = pow(2.0f, -Roughness);

	while (RectSize > 0)		//Se zema celiot teren prvo, pa na sekoja iteracija se deli na 4
	{
...
void MidpointDispTerrain::DiamondStep(int RectSize, float CurHeight) {		//se zema sekoe kjoshe, se naogja sredina i se pravi displecement
...
void MidpointDispTerrain::SquareStep(int RectSize, float CurHeight) {   //Se presmetuva visina na left & top mid edge na kvadratot (right & bottom se presmetuvat kaj sredn kvadrat), se pravi displacement
	int HalfRectSize = RectSize / 2;
